...
-- 2a. Si scriva una funzione height per calcolare l’altezza di un albero senza usare la treeFold dell’Esercizio 1.
height2 :: (Eq a, Show a) => Tree a -> Int
height2 = treeFold (\_ heights -> 1 + (if null heights then 0 else maximum heights)) (-1)
...
treefoldr f z g b (Vertex x ts) = f x (foldr (g . treefoldr f z g b) b ts)
...
height3 :: (Eq a, Show a) => Tree a -> Int
height3 = treefoldr (\_ h -> 1 + h) 0 max 0
...
-- usa la funzione treeFold per generare l'albero con tutti i nodi che hanno valore moltiplicato per 3
